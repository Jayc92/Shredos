...
-- 023_exlib_catalog_and_delivery_contract.sql   (REVISION E)
-- ============================================================
...
-- Snapshot immutability (Revision E, finding 3). Identity, content,
-- classification, provenance, and version columns are FROZEN after
-- insertion — corrections are represented by NEW version rows of the
-- same logical identity, never by editing a snapshot in place. Only
-- the review workflow columns (review_status, reviewed_by,
-- reviewed_at, review_rationale), is_active, and updated_at remain
-- mutable; those can only gate a row OUT of delivery, never alter
-- what it delivers. Freezing logical_id also makes stale claim
-- ownership structurally impossible: the claim triggers key on
-- (normalized_name, claim_source, logical_id), and neither component
-- can now drift underneath them.
...
-- Anatomy immutability + sealing (Revision E, finding 3). Anatomy
-- rows are never edited in place, and the anatomy SET of a snapshot
-- is sealed the moment the snapshot leaves pending review: what the
-- reviewer approved is exactly what every run can ever deliver.
-- Anatomy corrections, like every other content correction, are a
-- new catalog version row with its own anatomy.
...
BEGIN
  IF TG_OP = 'UPDATE' THEN
    RAISE EXCEPTION
      'exercise_catalog_run_items: membership rows are immutable; add or remove rows only while the run is unapproved';
  END IF;
  IF EXISTS (
    SELECT 1 FROM public.exercise_catalog_import_runs r
    WHERE r.id = COALESCE(NEW.run_id, OLD.run_id)
      AND r.approved_for_delivery = true
  ) THEN
    RAISE EXCEPTION
      'exercise_catalog_run_items: an approved run''s membership is frozen; revoking approval reopens editing but disables delivery';
  END IF;
  RETURN COALESCE(NEW, OLD);
END;
$$;
