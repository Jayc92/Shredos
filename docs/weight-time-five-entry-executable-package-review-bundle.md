...
| change surface | measured at the last gated commit `f14683e`: 19 paths, all under `docs/weight-time-five-entry-*` and `scripts/*five-entry*`; 0 paths under `src/`; 0 paths under `supabase/`. The freeze commit added this bundle as the 20th path. CORRECTED FORWARD (W14-E runtime evidence round, after the hosted run): a later successor adds exactly TWO more paths - `docs/weight-time-w14e-production-runtime-evidence-record.md`, the durable record of the Production fresh-account delivery smoke test, and `scripts/verify-weight-time-w14e-production-runtime-record.ts`, its static verifier - and B6's allowlist was widened BY NAME to admit exactly those two. So the invariant is not that the set never grows; it is that the set only ever grows by named paths that the endgame static change-surface census (B6) enforces against its allowlist on every run, still with 0 paths under `src/` |
