...
-- 023_exlib_catalog_and_delivery_contract.sql   (REVISION H)
-- ============================================================
...
--   Revision G (architecture APPROVED; superseded SOLELY by the
--   Revision H atomic-install transaction wrapper — not rejected):
...
-- Run AFTER 022.
--
-- Revision H correction (operational safety ONLY; every Revision G
-- schema, function, trigger, policy, grant, lifecycle, delivery,
-- rollback, approval, membership, and concurrency behavior is
-- preserved byte-for-byte apart from this wrapper and the version/
-- fingerprint documentation):
--   (H1) EXPLICIT TOP-LEVEL TRANSACTION: the migration no longer
--        relies on the executing client (Supabase SQL Editor or any
--        other) to provide atomic batch execution. One top-level
--        BEGIN; precedes the first executable statement and one
--        top-level COMMIT; follows the last — a failure at ANY
--        point rolls back the entire migration, leaving no
--        partially created tables, functions, policies, triggers,
--        grants, or altered exercises privileges. The BEGIN blocks
--        inside PL/pgSQL function bodies are code, not transaction
--        boundaries; there is exactly ONE top-level BEGIN and ONE
--        top-level COMMIT in this file.
--
...
-- Revision H: ONE explicit top-level transaction encloses every
-- executable statement below. Do not rely on any client to batch.
BEGIN;
...
-- Revision H: end of the single top-level transaction.
COMMIT;
