...
BEGIN
  v_result := public.load_catalog_content_draft(
    ...
    $br132$Take smaller breaths than usual and keep them continuous through the hold; never hold your breath to brace, because a long hold needs steady airflow.$br132$,
    $cm132$["Letting the hips drift up into a pike, which shortens the lever and makes the added plate easier than the logged weight suggests.","Placing the plate low on the lower back, where it loads the spine instead of the mid-back and hides a sagging position.","Reaching for a heavier plate before the unweighted hold is solid, so the position fails before the trunk is actually challenged."]$cm132$::jsonb,
  ...
  v_result := public.load_catalog_content_draft(
    ...
    $br133$Take smaller breaths than usual and keep them continuous; a snug vest limits how far the ribcage can expand, so do not brace by holding your breath.$br133$,
  ...
  v_result := public.load_catalog_content_draft(
    ...
    $es137$A secure pull-up bar rated for your bodyweight plus the added load, and a dipping belt with a weight plate, or a plate held securely between the feet.$es137$,
  ...
  v_result := public.load_catalog_content_draft(
    ...
    $br138$Keep breathing evenly the whole way through; the burn will tempt you to hold your breath, which ends the hold sooner than your legs would.$br138$,
    $cm138$["Sliding up out of parallel as the hold gets hard, which quietly makes the effort easier than the logged time suggests.","Resting the plate on the knees instead of near the hips, where it shifts the load and tends to slide.","Letting the knees travel forward past the toes, which moves the effort off the thighs.","Coming out of the hold by standing up with the plate still resting on the thighs."]$cm138$::jsonb,
  ...
  v_result := public.load_catalog_content_draft(
    ...
    $es139$A flat wall and a weighted vest with secured, evenly distributed pockets. Because the hands stay free, a vest suits longer holds than a plate held on the thighs.$es139$,
