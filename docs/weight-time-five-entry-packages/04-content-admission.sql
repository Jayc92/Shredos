-- ============================================================
-- W14-E stage 4 of 7 - CONTENT ADMISSION for the five weight_time identities
...
-- three human decision forms (families A and B gated (docs/weight-time-five-entry-snapshot-review-form.json, docs/weight-time-five-entry-content-review-form.json)).
--
...
--
-- POSITION IN THE SEQUENCE: stage 4 of 7. Vector before 8/8/10/3/11/6/2/2/1/6/8,
...
CREATE TEMP TABLE w14e4_capture ON COMMIT DROP AS
SELECT
...
BEGIN
  ...
  IF to_regprocedure('public.admit_catalog_content(uuid,uuid,text)') IS NULL THEN
    RAISE EXCEPTION 'W14E-4 content admission: migration-027 admit_catalog_content is missing at its exact signature; wrong or unmigrated database';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'exlib_catalog_admission') THEN
    RAISE EXCEPTION 'W14E-4 content admission: admission role missing';
  END IF;
  IF current_user <> 'postgres' OR session_user <> 'postgres' THEN
    RAISE EXCEPTION 'W14E-4 content admission: BOTH execution identities must be the hosted operator role postgres (got current_user=%, session_user=%); refusing before any write or authority change', current_user, session_user;
  END IF;
  IF (SELECT rolsuper FROM pg_roles WHERE rolname = current_user) THEN
    RAISE EXCEPTION 'W14E-4 content admission: the invoker is a superuser; this package is bound to the hosted non-superuser postgres posture';
  END IF;
  IF (SELECT count(*) FROM pg_catalog.pg_auth_members am
    ...
    RAISE EXCEPTION 'W14E-4 content admission: the exlib_catalog_admission membership posture is not the exact hosted baseline (exactly one membership: postgres granted BY supabase_admin with ADMIN TRUE, INHERIT FALSE, SET FALSE - grantor included); refusing before any write or authority change';
  END IF;
  SELECT
  ...
  IF v_counts <> '8/8/10/3/11/6/2/2/1/6/8' THEN
    RAISE EXCEPTION 'W14E-4 content admission: the catalog surface is not the exact expected pre-state (expected 8/8/10/3/11/6/2/2/1/6/8, found %); this ONE-USE package refuses to run twice, over foreign state, or over an ambiguous surface - READ STATE FIRST', v_counts;
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_import_runs r
    ...
    RAISE EXCEPTION 'W14E-4 content admission: the historical plank release run exlib2u-plank-release1-staged-v1 is not exactly one sealed, approved, non-dry, unrevoked run with six membership rows; the world is not the evidenced post-W14 hosted state; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content c
    ...
    RAISE EXCEPTION 'W14E-4 content admission: the published, admitted, fingerprint-fresh Plank content row is not exactly present; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_logical WHERE id IN ('e21b2c00-0000-4000-a000-000000000009', 'e21b2c00-0000-4000-a000-00000000000a', 'e21b2c00-0000-4000-a000-00000000000b')) <> 0
    ...
    RAISE EXCEPTION 'W14E-4 content admission: a deferred carry identity (inventory lines 134, 135, 136) is present in the catalog; the carries are DEFERRED and this lifecycle never touches them; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog e
    ...
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 132 (Plate-weighted plank) is not exactly one active v1 snapshot carrying the governed W14 fields; refusing';
  END IF;
  IF (SELECT coalesce(string_agg(m.muscle || ':' || m.role, ',' ORDER BY m.muscle, m.role), '')
    ...
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 132 anatomy is not exactly obliques:secondary; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_aliases a WHERE a.logical_id = 'e21b2c00-0000-4000-a000-000000000004') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 132 alias count is not 0; refusing';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
    ...
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 132 does not bear the exact family A approval tuple; refusing';
  END IF;
  ...
  IF (SELECT count(*) FROM public.exercise_catalog_content_expected_relationships x WHERE x.content_id = 'e21b2c00-0000-4000-a000-000000000104') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: the expected-relationship set for inventory line 132 drifted; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog e
     ...
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000005' AND e.is_active = true
         ...
         AND e.import_confidence = 'human_review_required'
         AND e.catalog_version = 1) THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 133 (Weighted vest plank) is not exactly one active v1 snapshot carrying the governed W14 fields; refusing';
  END IF;
  IF (SELECT coalesce(string_agg(m.muscle || ':' || m.role, ',' ORDER BY m.muscle, m.role), '')
        FROM public.exercise_catalog_muscles m
        JOIN public.exercise_catalog e ON e.id = m.catalog_id
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000005' AND e.is_active = true) <> 'obliques:secondary' THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 133 anatomy is not exactly obliques:secondary; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_aliases a WHERE a.logical_id = 'e21b2c00-0000-4000-a000-000000000005') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 133 alias count is not 0; refusing';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000005' AND e.is_active = true
         AND e.review_status = 'approved'
    ...
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 133 does not bear the exact family A approval tuple; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content c WHERE c.logical_id = 'e21b2c00-0000-4000-a000-000000000005') <> 1
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000105'
         ...
         AND c.import_admitted = false
         AND c.admitted_fingerprint IS NULL AND c.admitted_source_sha256 IS NULL AND c.admitted_at IS NULL) THEN
    RAISE EXCEPTION 'W14E-4 content admission: the content row for inventory line 133 is not the exact reviewed pre-admission state (payload, the applied family B tuple, or the approved/draft/UNADMITTED lifecycle drifted - a second admission is refused by design; READ STATE FIRST); refusing before any write or authority change';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content_expected_relationships x WHERE x.content_id = 'e21b2c00-0000-4000-a000-000000000105') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: the expected-relationship set for inventory line 133 drifted; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog e
     ...
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000006' AND e.is_active = true
         ...
         AND e.catalog_version = 1) THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 137 (Weighted dead hang) is not exactly one active v1 snapshot carrying the governed W14 fields; refusing';
  END IF;
  IF (SELECT coalesce(string_agg(m.muscle || ':' || m.role, ',' ORDER BY m.muscle, m.role), '')
        FROM public.exercise_catalog_muscles m
        JOIN public.exercise_catalog e ON e.id = m.catalog_id
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000006' AND e.is_active = true) <> 'lats:secondary' THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 137 anatomy is not exactly lats:secondary; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_aliases a WHERE a.logical_id = 'e21b2c00-0000-4000-a000-000000000006') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 137 alias count is not 0; refusing';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000006' AND e.is_active = true
         AND e.review_status = 'approved'
    ...
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 137 does not bear the exact family A approval tuple; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content c WHERE c.logical_id = 'e21b2c00-0000-4000-a000-000000000006') <> 1
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000106'
         ...
         AND c.import_admitted = false
         AND c.admitted_fingerprint IS NULL AND c.admitted_source_sha256 IS NULL AND c.admitted_at IS NULL) THEN
    RAISE EXCEPTION 'W14E-4 content admission: the content row for inventory line 137 is not the exact reviewed pre-admission state (payload, the applied family B tuple, or the approved/draft/UNADMITTED lifecycle drifted - a second admission is refused by design; READ STATE FIRST); refusing before any write or authority change';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content_expected_relationships x WHERE x.content_id = 'e21b2c00-0000-4000-a000-000000000106') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: the expected-relationship set for inventory line 137 drifted; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog e
     ...
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000007' AND e.is_active = true
         ...
         AND e.import_confidence IS NULL
         AND e.catalog_version = 1) THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 138 (Weighted wall sit) is not exactly one active v1 snapshot carrying the governed W14 fields; refusing';
  END IF;
  IF (SELECT coalesce(string_agg(m.muscle || ':' || m.role, ',' ORDER BY m.muscle, m.role), '')
        FROM public.exercise_catalog_muscles m
        JOIN public.exercise_catalog e ON e.id = m.catalog_id
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000007' AND e.is_active = true) <> 'glutes:secondary' THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 138 anatomy is not exactly glutes:secondary; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_aliases a WHERE a.logical_id = 'e21b2c00-0000-4000-a000-000000000007') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 138 alias count is not 0; refusing';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000007' AND e.is_active = true
         AND e.review_status = 'approved'
    ...
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 138 does not bear the exact family A approval tuple; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content c WHERE c.logical_id = 'e21b2c00-0000-4000-a000-000000000007') <> 1
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000107'
         ...
         AND c.import_admitted = false
         AND c.admitted_fingerprint IS NULL AND c.admitted_source_sha256 IS NULL AND c.admitted_at IS NULL) THEN
    RAISE EXCEPTION 'W14E-4 content admission: the content row for inventory line 138 is not the exact reviewed pre-admission state (payload, the applied family B tuple, or the approved/draft/UNADMITTED lifecycle drifted - a second admission is refused by design; READ STATE FIRST); refusing before any write or authority change';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content_expected_relationships x WHERE x.content_id = 'e21b2c00-0000-4000-a000-000000000107') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: the expected-relationship set for inventory line 138 drifted; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog e
     ...
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000008' AND e.is_active = true
         ...
         AND e.import_confidence IS NULL
         AND e.catalog_version = 1) THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 139 (Weighted vest wall sit) is not exactly one active v1 snapshot carrying the governed W14 fields; refusing';
  END IF;
  IF (SELECT coalesce(string_agg(m.muscle || ':' || m.role, ',' ORDER BY m.muscle, m.role), '')
        FROM public.exercise_catalog_muscles m
        JOIN public.exercise_catalog e ON e.id = m.catalog_id
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000008' AND e.is_active = true) <> 'glutes:secondary' THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 139 anatomy is not exactly glutes:secondary; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_aliases a WHERE a.logical_id = 'e21b2c00-0000-4000-a000-000000000008') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 139 alias count is not 0; refusing';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog e
       WHERE e.logical_id = 'e21b2c00-0000-4000-a000-000000000008' AND e.is_active = true
         AND e.review_status = 'approved'
    ...
    RAISE EXCEPTION 'W14E-4 content admission: inventory line 139 does not bear the exact family A approval tuple; refusing';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content c WHERE c.logical_id = 'e21b2c00-0000-4000-a000-000000000008') <> 1
     OR NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000108'
         ...
         AND c.import_admitted = false
         AND c.admitted_fingerprint IS NULL AND c.admitted_source_sha256 IS NULL AND c.admitted_at IS NULL) THEN
    RAISE EXCEPTION 'W14E-4 content admission: the content row for inventory line 139 is not the exact reviewed pre-admission state (payload, the applied family B tuple, or the approved/draft/UNADMITTED lifecycle drifted - a second admission is refused by design; READ STATE FIRST); refusing before any write or authority change';
  END IF;
  IF (SELECT count(*) FROM public.exercise_catalog_content_expected_relationships x WHERE x.content_id = 'e21b2c00-0000-4000-a000-000000000108') <> 0 THEN
    RAISE EXCEPTION 'W14E-4 content admission: the expected-relationship set for inventory line 139 drifted; refusing';
  END IF;
  IF (SELECT orphaned_claims::text || '/' || unclaimed_bearers::text FROM public.exlib_verify_catalog_claims()) <> '0/0' THEN
    RAISE EXCEPTION 'W14E-4 content admission: the bidirectional catalog name-claim invariant does not hold; refusing';
  END IF;
  IF has_function_privilege('anon', 'public.admit_catalog_content(uuid,uuid,text)', 'EXECUTE')
    ...
    RAISE EXCEPTION 'W14E-4 content admission: public.admit_catalog_content(uuid,uuid,text) is executable by an ordinary client role; refusing before any write or authority change';
  END IF;
END
$pre$;
...
BEGIN
  IF (SELECT count(*) FROM pg_catalog.pg_auth_members am
    ...
    RAISE EXCEPTION 'W14E-4 content admission: the two-grantor membership shape after the temporary grant is not exact (supabase_admin-granted baseline row plus postgres-granted SET row); aborting before SET ROLE and before any call';
  END IF;
END
$auth$;
...
BEGIN
  ...
  IF v_result IS DISTINCT FROM jsonb_build_object(
       ...
       'content_version', 1,
       'admitted_fingerprint', '9cbc10c9284f3e23f1123b647f17ee6bc05e8e452f5aa821b9a99256c9ddae7c',
  ...
  IF v_result IS DISTINCT FROM jsonb_build_object(
       ...
       'content_version', 1,
       'admitted_fingerprint', 'bb705be0318c34b7fd2ecd51039a665ad087be8f69fc227cf44a53ddbb06f1a5',
       'admitted_source_sha256', '8fa1d3402a3ca9beef8b1cbb7ba58692bea0d28c11926d6db33da72877f2afdb') THEN
  ...
  IF v_result IS DISTINCT FROM jsonb_build_object(
       ...
       'content_version', 1,
       'admitted_fingerprint', 'e10369c291030ed61ddc48eda0c5c8759e0f3a7c68229a19a9b7c09792fe2008',
       'admitted_source_sha256', '8fa1d3402a3ca9beef8b1cbb7ba58692bea0d28c11926d6db33da72877f2afdb') THEN
  ...
  IF v_result IS DISTINCT FROM jsonb_build_object(
       ...
       'content_version', 1,
       'admitted_fingerprint', '05ca70e920ac098f291c9928210f724ba15044bab7fac588591026b3d9d2b932',
       'admitted_source_sha256', '8fa1d3402a3ca9beef8b1cbb7ba58692bea0d28c11926d6db33da72877f2afdb') THEN
  ...
  IF v_result IS DISTINCT FROM jsonb_build_object(
       ...
       'content_version', 1,
       'admitted_fingerprint', '8a7a94b2ede86cae694cde02fa5652154204a2093562f45c54778d0c2ff68c65',
       'admitted_source_sha256', '8fa1d3402a3ca9beef8b1cbb7ba58692bea0d28c11926d6db33da72877f2afdb') THEN
...
BEGIN
  ...
  IF (SELECT count(*) FROM pg_catalog.pg_auth_members am
    ...
    RAISE EXCEPTION 'W14E-4 content admission: authority restoration is not exact (baseline row plus zero standing SET capability required); rolling back everything';
  END IF;
  SELECT
    (SELECT count(*) FROM public.exercise_catalog_logical)::text
    || '/' || (SELECT count(*) FROM public.exercise_catalog)::text
    ...
    INTO v_counts;
  IF v_counts <> '8/8/10/3/11/6/2/2/1/6/8' THEN
    RAISE EXCEPTION 'W14E-4 content admission: post-state vector is % (expected 8/8/10/3/11/6/2/2/1/6/8); rolling back everything', v_counts;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
    ...
    RAISE EXCEPTION 'W14E-4 content admission: the admitted content row for inventory line 132 is not exact (admission surface, precomputed fingerprint equality, fresh recomputation equality, audit tuple, frozen payload, or draft publication drifted); rolling back everything';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000105'
         AND c.logical_id = 'e21b2c00-0000-4000-a000-000000000005'
         ...
         AND c.admitted_at = CURRENT_DATE
         AND c.publication_status = 'draft') THEN
    RAISE EXCEPTION 'W14E-4 content admission: the admitted content row for inventory line 133 is not exact (admission surface, precomputed fingerprint equality, fresh recomputation equality, audit tuple, frozen payload, or draft publication drifted); rolling back everything';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000106'
         AND c.logical_id = 'e21b2c00-0000-4000-a000-000000000006'
         ...
         AND c.admitted_at = CURRENT_DATE
         AND c.publication_status = 'draft') THEN
    RAISE EXCEPTION 'W14E-4 content admission: the admitted content row for inventory line 137 is not exact (admission surface, precomputed fingerprint equality, fresh recomputation equality, audit tuple, frozen payload, or draft publication drifted); rolling back everything';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000107'
         AND c.logical_id = 'e21b2c00-0000-4000-a000-000000000007'
         ...
         AND c.admitted_at = CURRENT_DATE
         AND c.publication_status = 'draft') THEN
    RAISE EXCEPTION 'W14E-4 content admission: the admitted content row for inventory line 138 is not exact (admission surface, precomputed fingerprint equality, fresh recomputation equality, audit tuple, frozen payload, or draft publication drifted); rolling back everything';
  END IF;
  IF NOT EXISTS (SELECT 1 FROM public.exercise_catalog_content c
       WHERE c.id = 'e21b2c00-0000-4000-a000-000000000108'
         AND c.logical_id = 'e21b2c00-0000-4000-a000-000000000008'
         ...
         AND c.admitted_at = CURRENT_DATE
         AND c.publication_status = 'draft') THEN
    RAISE EXCEPTION 'W14E-4 content admission: the admitted content row for inventory line 139 is not exact (admission surface, precomputed fingerprint equality, fresh recomputation equality, audit tuple, frozen payload, or draft publication drifted); rolling back everything';
  END IF;
  IF (SELECT md5(coalesce(string_agg(l::text, '|' ORDER BY l.id), '-')) FROM public.exercise_catalog_logical l) IS DISTINCT FROM v_cap.logical_digest
    ...
    RAISE EXCEPTION 'W14E-4 content admission: a surface this package must not change has changed (logical_digest, snapshots_digest, events_digest, anatomy_digest, alias_digest, claims_digest, content_outside_five, expected_rel_digest, projection_digest, runs_digest, run_items_digest); rolling back everything';
  END IF;
  IF (SELECT r::text FROM public.exercise_catalog_import_runs r WHERE r.run_key = 'exlib2u-plank-release1-staged-v1') IS DISTINCT FROM v_cap.historical_run_row
    ...
    RAISE EXCEPTION 'W14E-4 content admission: the historical sealed plank run exlib2u-plank-release1-staged-v1 or its membership changed; this package never touches it; rolling back everything';
  END IF;
  IF (SELECT md5(coalesce(string_agg(am::text, '|' ORDER BY am.roleid, am.member, am.grantor), '-'))
    ...
    RAISE EXCEPTION 'W14E-4 content admission: the catalog authority memberships changed across the act - whole rows compared: member, grantor, every option column; rolling back everything';
  END IF;
  IF (SELECT count(*)::text || ':' || md5(coalesce(string_agg(t::text, '|' ORDER BY t.id), '-')) FROM public.exercises t) IS DISTINCT FROM v_cap.tenant_digest
    ...
    RAISE EXCEPTION 'W14E-4 content admission: a tenant surface (exercises, exercise_aliases, exercise_muscles) changed inside the gated interval; NO tenant delivery occurs in this package; rolling back everything';
  END IF;
  IF (SELECT orphaned_claims::text || '/' || unclaimed_bearers::text FROM public.exlib_verify_catalog_claims()) <> '0/0' THEN
    RAISE EXCEPTION 'W14E-4 content admission: the bidirectional catalog name-claim invariant does not hold; rolling back everything';
  END IF;
  IF has_function_privilege('anon', 'public.admit_catalog_content(uuid,uuid,text)', 'EXECUTE')
     OR has_function_privilege('authenticated', 'public.admit_catalog_content(uuid,uuid,text)', 'EXECUTE')
     OR has_function_privilege('service_role', 'public.admit_catalog_content(uuid,uuid,text)', 'EXECUTE') THEN
    RAISE EXCEPTION 'W14E-4 content admission: public.admit_catalog_content(uuid,uuid,text) is executable by an ordinary client role; rolling back everything';
  END IF;
END
$post$;

-- surfaced result (display evidence; the committed rows are the proof)
SELECT 'W14E-4 CONTENT ADMITTED' AS result,
       (SELECT
          (SELECT count(*) FROM public.exercise_catalog_logical)::text
          || '/' || (SELECT count(*) FROM public.exercise_catalog)::text
          || '/' || (SELECT count(*) FROM public.exercise_catalog_muscles)::text
       ...
       (SELECT count(*) FROM public.exercise_catalog_content WHERE logical_id IN ('e21b2c00-0000-4000-a000-000000000004', 'e21b2c00-0000-4000-a000-000000000005', 'e21b2c00-0000-4000-a000-000000000006', 'e21b2c00-0000-4000-a000-000000000007', 'e21b2c00-0000-4000-a000-000000000008') AND import_admitted) AS admitted_content;
