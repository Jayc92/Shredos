...
async function main(): Promise<void> {
  ...
  console.log('\nD. Authority, advisors, lifecycle distinction, and boundaries')
  {
    ...
    check('D4: boundaries hold — the frozen set is blob-identical to the promoted tip (seed module, inventory, ledger, package.json, both batch artifacts), the Plank inventory row stays seed_link_compatible false, and the phase range through the anchored delivery-runtime predecessor touches only docs/ and scripts/verify-* paths (RETARGET (EXLIB-2T delivery-runtime preparation))',
      (() => {
