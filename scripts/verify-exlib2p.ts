// EXLIB-2P STATIC verification (LOCAL-ONLY): the prepared Plank
// DATABASE CONTENT-REVIEW package, its preparation record, and the
// phase lifecycle.
//
// Everything the package claims is INDEPENDENTLY RE-DERIVED here from
// the promoted sources: the human tuple from the completed EXLIB-2H/2I
// review form AND the admitted artifact's content_review object; every
// payload literal from the admitted artifact field by field (JSONB
// fields compared as parsed values, text fields as exact strings, no
// hashes); the target bindings from the completed 2N category forms
// and the admitted batch lines; the Plank snapshot vocabulary from the
// SPENT EXLIB-2K package's own dollar-quoted literals. The package's
// single review call is proven SCHEMA-QUALIFIED with ZERO unqualified
// call sites and ZERO admission/publication call sites; both vector-
// building queries are mechanically extracted and proven identical in
// the canonical eleven-table order; the authority dance (baseline
// gate, two-grantor proof, SET ROLE, grantor-scoped REVOKE,
// restoration) is proven structurally; the review-event SCOPING
// derived from migrations 023/027 (snapshot-scoped, trigger-internal,
// depth >= 2) is proven disclosed in both the package and the record.
// Performs NO hosted contact and executes nothing.
//
...
const PKG = 'docs/exlib2p-plank-database-review-package.sql'
const RECORD = 'docs/exlib2p-plank-database-review-prep-record.md'
const VERIFIER = 'scripts/verify-exlib2p.ts'
const LIVE = 'scripts/verify-exlib2p-live.sh'
const RETARGETED = 'scripts/verify-exlib2o-application.ts'
...
const SRC = '442b6247ad2f4b95ce58a1c2ed72df2ca84aff63'
const SRC_TREE = 'aee2a0c72c2fdcd8b9aa8f505c71cbf235e42252'
const SRC_TAG = 'exlib2o-hosted-load-application-evidence-stable'
const SRC_TAG_OBJ = 'd244aa4a27efd34fec489fc0087c34c03d2e561d'
const SRC_TAG_MSG = 'EXLIB-2O target-snapshot hosted-load application evidence — LOADED — TARGET GATE SATISFIED\n'
...
const FPS: Array<[string, number, string]> = [
  ...
  [REC2O, 15938, 'e45939733abda83932173c492a6436aca5e188bd2644f34f87f3a03175edea09'],
...
check('B4: exactly ONE SCHEMA-QUALIFIED public.apply_content_review call with the parsed 6-argument arity, the qualified to_regprocedure precondition, ZERO unqualified call sites, and ZERO admission/publication call sites',
  (() => {
    const calls = pkg.match(/^SELECT public\.apply_content_review\(([\s\S]*?)\);$/gm)
    ...
    if (!pkg.includes(`to_regprocedure('public.apply_content_review(${REVIEW_SIG})')`)) return false
    const unqual = (pkg.match(/(^|[^.a-zA-Z0-9_])apply_content_review\s*\(/gm) || [])
    if (unqual.length !== 0) return false
    return !/admit_catalog_content\s*\(/.test(pkg) && !/publish_catalog_content\s*\(/.test(pkg)
  })())
...
check('B6: the authority dance is complete and structural — grantor-included baseline gate, GRANT ... WITH SET TRUE INHERIT FALSE, the exact two-grantor proof BEFORE SET ROLE, SET ROLE/RESET ROLE bracketing the single call, the grantor-scoped REVOKE, and the restoration postcondition with pg_has_role SET false',
  pkgFlat.includes("r.rolname = 'exlib_catalog_reviewer' AND m.rolname = 'postgres' AND g.rolname = 'supabase_admin' AND am.admin_option AND NOT am.inherit_option AND NOT am.set_option")
  && pkg.includes('GRANT exlib_catalog_reviewer TO postgres WITH SET TRUE, INHERIT FALSE;')
  ...
  && pkgFlat.includes("pg_has_role('postgres', 'exlib_catalog_reviewer', 'SET')")
  && pkg.indexOf('SET ROLE exlib_catalog_reviewer;') < pkg.indexOf('SELECT public.apply_content_review(')
  && pkg.indexOf('SELECT public.apply_content_review(') < pkg.indexOf('RESET ROLE;'))
check('B7: the review-event SCOPING is enforced and disclosed — the package asserts ZERO Plank review events before, ZERO review events after, and carries the snapshot-scoped/trigger-internal derivation so "no event" reads as schema design, never as missing evidence',
  ...
  && pkgFlat.includes('a review event appeared; the snapshot-scoped log must stay empty under a content review')
...
check('B8: ordinary clients stay locked out — has_function_privilege is checked for anon, authenticated, AND service_role, in BOTH the preconditions and the postconditions',
  (pkg.match(new RegExp(`has_function_privilege\\('(anon|authenticated|service_role)', 'public\\.apply_content_review\\(${REVIEW_SIG.replace(/,/g, ',')}\\)', 'EXECUTE'\\)`, 'g')) || []).length === 6)
check('B9: the dual-identity and non-superuser gates run before any authority change, and every gate failure message names the refusal',
  ...
  && pkg.indexOf("current_user <> 'postgres'") < pkg.indexOf('GRANT exlib_catalog_reviewer'))

console.log('\nC. Literals re-derived from the promoted sources')
check('C1: the human tuple is VERBATIM — decision, reviewer, timestamp, and rationale in the call equal the completed form AND the admitted artifact\'s content_review object, and the post-gate pins the same tuple with the timestamptz INSTANT',
  lit(pkg, 'dec') === 'approved' && form.decision === 'approved' && art.content_review.status === 'approved'
  && lit(pkg, 'rev') === 'Nick Tkacz' && form.reviewer === 'Nick Tkacz' && art.content_review.reviewer === 'Nick Tkacz'
  && pkg.includes("TIMESTAMPTZ '2026-09-01T20:35:00-04:00'")
  && form.reviewed_at === '2026-09-01T20:35:00-04:00' && art.content_review.reviewed_at === '2026-09-01T20:35:00-04:00'
  && lit(pkg, 'rat') === 'Everything looks correct' && form.rationale === 'Everything looks correct'
  && art.content_review.rationale === 'Everything looks correct'
  && lit(pkg, 'q_rev') === 'Nick Tkacz' && lit(pkg, 'q_rat') === 'Everything looks correct'
  && (pkg.match(/TIMESTAMPTZ '2026-09-01T20:35:00-04:00'/g) || []).length === 2)
...
check('C6: the content-identity and lifecycle pins are exact — content ...0101 under logical ...0001 version 1, pending/draft/unadmitted with NULL evidence in the pre-gate, approved/draft/unadmitted with the exact tuple in the post-gate',
  pkgFlat.includes(`c.id = '${CV}' AND c.logical_id = '${PL}' AND c.content_version = 1`)
  && pkgFlat.includes("c.content_status = 'pending' AND c.publication_status = 'draft' AND c.import_admitted = false")
  && pkgFlat.includes('c.reviewed_by IS NULL AND c.reviewed_at IS NULL AND c.review_rationale IS NULL')
  && pkgFlat.includes("c.content_status = 'approved'")
  && pkgFlat.includes('c.admitted_fingerprint IS NULL AND c.admitted_source_sha256 IS NULL AND c.admitted_at IS NULL'))
...
check('D5: honesty and boundaries — ONE-USE not idempotent with the mechanical reason, why this milestone changes NO hosted state, the complete dependency map, and the derived contract (function, signature, caller role, decision vocabulary, one-way machine, fresh-tuple rule)',
  recFlat.includes('ONE-USE') && recFlat.includes('not idempotent')
  && recFlat.includes('changes no hosted state')
  && recFlat.includes('apply_content_review') && recFlat.includes(REVIEW_SIG)
  && recFlat.includes('exlib_catalog_reviewer')
  && recFlat.includes("approved, revised, or rejected")
  && recFlat.includes('one-way') && recFlat.includes('SECURITY DEFINER'))
...
check('E2: the derived contract matches the migration bytes — apply_content_review with the exact 6-argument signature, SECURITY DEFINER, EXECUTE granted ONLY to exlib_catalog_reviewer with PUBLIC/anon/authenticated revoked, the pending-only one-time rule, and the approved/revised/rejected vocabulary',
  (() => {
    const mig = read(MIG027)
    return mig.includes('CREATE OR REPLACE FUNCTION apply_content_review(')
      && mig.includes('GRANT EXECUTE ON FUNCTION apply_content_review(UUID, UUID, TEXT, TEXT, TIMESTAMPTZ, TEXT) TO exlib_catalog_reviewer;')
      && mig.includes('REVOKE EXECUTE ON FUNCTION apply_content_review(UUID, UUID, TEXT, TEXT, TIMESTAMPTZ, TEXT) FROM PUBLIC;')
      && mig.includes('only a pending version can receive its review decision through this authority')
      && mig.includes("p_decision NOT IN ('approved','revised','rejected')")
      && read(MIG023).includes('events are written only by the snapshot review transition trigger')
  })())
check('E3: the EXLIB-2O application suite is retargeted under the exact EXLIB-2P label — the label appears at least twice, the evidence tip and its tree are pinned, and E5 walks the tip rather than HEAD',
  (() => {
    const r = bytesOf(RETARGETED).toString('utf8')
    if ((r.match(new RegExp(LABEL_2P.replace(/[()]/g, '\\$&'), 'g')) ?? []).length < 2) return false
    if (!r.includes(`const EV_TIP = '${SRC}'`)) return false
    if (!r.includes(`const EV_TREE = '${SRC_TREE}'`)) return false
    return r.includes('rev-list --count ${R1}..${EV_TIP}')
  })())

console.log('\nF. The live verifier, mechanically')
check('F1: the live verifier exists, targets only disposable socket-only clusters, builds the pre-state by executing the SPENT 2K and 2O packages, carries the full refusal matrix through checked fail-loud surgeries, proves the one-use content gate, the race, and the same-signature search_path decoy on durable function statistics',
  (() => {
    const l = bytesOf(LIVE).toString('utf8')
    if (!l.includes('unix_socket_directories') || !l.includes("listen_addresses=''")) return false
    if (!l.includes('-f "$PKG2K"') || !l.includes('-f "$PKG2O"')) return false
    if ((l.match(/^expect_pkg_refusal /gm) || []).length !== 10) return false
    if (!l.includes('HARNESS SURGERY FAILED')) return false
    if (!l.includes('not the exact loaded pre-review state')) return false
    if (!l.includes('track_functions=all') || !l.includes('pg_stat_user_functions')) return false
    if (!l.includes('CREATE FUNCTION exlib2p_decoy.apply_content_review(')) return false
    if (!l.includes('ALTER DATABASE $V SET search_path = exlib2p_decoy, public')) return false
    if (!l.includes(`CONTENT_POST="approved|Nick Tkacz|2026-09-02T00:35:00Z|Everything looks correct|draft|false"`)) return false
    if (!l.includes('ROLE MEMBERSHIPS ARE CLUSTER-WIDE')) return false
    return !/supabase\.co|vercel\./.test(l)
  })())
check('F2: the live one-use and race proofs key on the CONTENT gate, not the vector — the script states why (a review changes no count) and greps for the content-pending refusal in both places',
  (() => {
    const l = bytesOf(LIVE).toString('utf8')
    return l.includes('the vector alone cannot catch')
      && (l.match(/not the exact loaded pre-review state/g) || []).length >= 3
  })())
...
check('G3: two-state lifecycle — the package, record, and both verifiers are absent at the promoted source tip and present in this phase',
  (() => {
    ...
    if (srcDocs.includes('exlib2p-')) return false
    if (srcScripts.includes('verify-exlib2p')) return false
...
check('H2: the frozen product surface held through this milestone (delivery paths anchored at the delivery predecessor) — seed module, inventory, review ledger, package.json, and the batch artifacts are blob-identical to the promoted source tip',
  (() => {
