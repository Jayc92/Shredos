        ...
        try {
          ...
          const dirtyAfterB3 = execSync('git status --porcelain', { encoding: 'utf8' })
            ...
            .filter((l) => {
              ...
              if (st === 'M' && f.startsWith('scripts/verify-') && f.endsWith('.ts')) {
                try {
                  return !/ADMISSION \(EXLIB-1C0B3\)|RETARGET \(EXLIB-1C0B3 migration 025 draft\)|ADMISSION \(EXLIB-1C0B4 weight_time product decisions\)|ADMISSION \(EXLIB-1C0B5 weight_time rpe-warmup decision\)/.test(
                    execSync(`git diff -- ${f}`, { encoding: 'utf8' }))
                } catch { return true }
              }
              return true
