...
async function main(): Promise<void> {
  ...
  console.log('\nA. Baseline and phase boundary')
  {
    ...
    // RETARGET (EXLIB-2C batch 3): the BATCH 2 milestone's range
    // claim is anchored to its own promoted tip (6ca5b14...), not to
    // a moving HEAD, so later authoring batches building on the
    // promoted Batch 2 can never dilute or break this historical
    // claim. HEAD must still descend from that tip once it exists.
    check('A2: planning-only boundary — migration 026 absent, migrations exactly 001-025, zero weight_time in src, no importer artifacts, and the BATCH 2 range (batch 1 tip..batch 2 tip) touches ONLY this phase\'s five paths',
      (() => {
        ...
        const inHistory = (() => {
          try {
            execSync(`git cat-file -e ${BATCH2_TIP}^{commit}`, { stdio: 'pipe' })
            return true
          } catch { return false }
        })()
        if (!inHistory) {
        ...
        execSync(`git merge-base --is-ancestor ${BATCH2_TIP} HEAD`)
        const range = execSync(`git diff --name-only ${BATCH1_TIP}..${BATCH2_TIP}`, { encoding: 'utf8' })
          .split('\n').filter(Boolean).sort()
