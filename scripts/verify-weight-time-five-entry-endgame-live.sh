...
# stage 6
run_stage 6 "$S6"
expect_eq "E6.1: vector moved exactly runs 1 -> 2 and run_items 6 -> 11: $V6" "$VECTOR_SQL" "$V6"
expect_eq "E6.2: the new run exists under the proposed key, unsealed / unapproved / unrevoked / non-dry with the synthetic family C evidence, five exercise members, zero alias members, and does NOT satisfy the delivery predicate" \
  "SELECT (SELECT count(*) FROM public.exercise_catalog_import_runs r WHERE r.run_key='$NEW_KEY' AND NOT r.approved_for_delivery AND r.sealed_at IS NULL AND r.revoked_at IS NULL AND NOT r.dry_run AND r.product_approved_by='TEST-ONLY Approver (synthetic)' AND r.legal_approved_at=TIMESTAMPTZ '2026-09-12T10:10:00-04:00')::text||'/'||(SELECT count(*) FROM public.exercise_catalog_run_items ri JOIN public.exercise_catalog_import_runs r ON r.id=ri.run_id JOIN public.exercise_catalog c ON c.id=ri.catalog_id WHERE r.run_key='$NEW_KEY' AND c.logical_id IN ($FIVE))::text||'/'||(SELECT count(*) FROM public.exercise_catalog_run_items ri JOIN public.exercise_catalog_import_runs r ON r.id=ri.run_id WHERE r.run_key='$NEW_KEY' AND ri.catalog_alias_id IS NOT NULL)::text||'/'||(SELECT count(*) FROM public.exercise_catalog_import_runs r WHERE r.run_key='$NEW_KEY' AND r.approved_for_delivery AND NOT r.dry_run AND r.sealed_at IS NOT NULL AND r.revoked_at IS NULL)::text" "1/5/0/0"
expect_eq "E6.3: the new key is NOT the historical key, and the historical run is byte-identical to the pre-stage capture" \
  "SELECT ('$NEW_KEY' <> '$HIST_KEY')::text||'#'||($HIST_RUN_SQL)" "true#$HIST_BEFORE"
# stage 7
run_stage 7 "$S7"
expect_eq "E7.1: vector unchanged: $V6" "$VECTOR_SQL" "$V6"
expect_eq "E7.2: the new run is sealed, approved, unrevoked; the delivery predicate now matches EXACTLY TWO runs (historical + new); no tenant row carries the new run yet" \
...
echo
echo "=== F. Tenant delivery (the per-user runtime RPC), proven for a fixture user; the plank run unaffected"
U1="${UIDS[0]}"; U2="${UIDS[1]}"
DELIV=$(QQ "SET app.uid = '$U1'; SELECT (public.deliver_catalog_exercises('$NEW_KEY') IS NOT DISTINCT FROM jsonb_build_object('run_key','$NEW_KEY','eligible',5,'inserted',5,'skipped_already_delivered',0,'skipped_name_collision',0,'collision_names','[]'::jsonb,'alias_inserted',0,'alias_added_to_existing',0,'alias_already_delivered',0,'alias_skipped_no_exercise',0,'alias_skipped_inactive_exercise',0,'alias_skipped_collision',0,'inserted_catalog_logical_ids',jsonb_build_array('$U132','$U137','$U133','$U139','$U138'),'plank_disposition','not_in_run'))::text")
[ "$DELIV" = "true" ] && ok "F1: deliver_catalog_exercises(new key) for user 1 returned EXACTLY the manifest's expected summary - eligible 5, inserted 5, every other counter 0, the five ids in lower(canonical_name) order, plank_disposition not_in_run, accounting offset 0" \
  || bad "F1: delivery summary differs" "$DELIV $(QQ "SET app.uid = '$U1'; SELECT public.deliver_catalog_exercises('$NEW_KEY')::text" | head -c 400)"
expect_eq "F2: user 1 now holds exactly five new system exercises linked to the five identities and the new run - tracking_mode weight_time, exercise_type strength, unilateral false - with five exercise_muscles rows; user 1 total 21 -> 26" \
  "SELECT (SELECT count(*) FROM public.exercises e JOIN public.exercise_catalog_import_runs r ON r.id=e.import_run_id WHERE e.user_id='$U1' AND r.run_key='$NEW_KEY' AND e.catalog_logical_id IN ($FIVE) AND e.tracking_mode='weight_time' AND e.exercise_type='strength' AND e.unilateral=false AND e.is_system AND e.is_active AND e.catalog_id IS NOT NULL)::text||'/'||(SELECT count(*) FROM public.exercise_muscles m JOIN public.exercises e ON e.id=m.exercise_id WHERE e.user_id='$U1' AND e.catalog_logical_id IN ($FIVE))::text||'/'||(SELECT count(*) FROM public.exercises WHERE user_id='$U1')::text" "5/5/26"
expect_eq "F3: the delivered names are the five canonical names, each carrying the catalog snapshot's category / primary_muscle / equipment" \
  "SELECT string_agg(e.name||':'||e.category||':'||e.primary_muscle||':'||e.equipment, ';' ORDER BY e.name) FROM public.exercises e WHERE e.user_id='$U1' AND e.catalog_logical_id IN ($FIVE)" \
  "Plate-weighted plank:isolation:abs:weight_plate;Weighted dead hang:isolation:forearms:weight_plate;Weighted vest plank:isolation:abs:weighted_vest;Weighted vest wall sit:compound:quads:weighted_vest;Weighted wall sit:compound:quads:weight_plate"
DELIV2=$(QQ "SET app.uid = '$U1'; SELECT (public.deliver_catalog_exercises('$NEW_KEY') IS NOT DISTINCT FROM jsonb_build_object('run_key','$NEW_KEY','eligible',5,'inserted',0,'skipped_already_delivered',5,'skipped_name_collision',0,'collision_names','[]'::jsonb,'alias_inserted',0,'alias_added_to_existing',0,'alias_already_delivered',0,'alias_skipped_no_exercise',0,'alias_skipped_inactive_exercise',0,'alias_skipped_collision',0,'inserted_catalog_logical_ids','[]'::jsonb,'plank_disposition','not_in_run'))::text")
[ "$DELIV2" = "true" ] && ok "F4: a SECOND delivery for the same user is idempotent - skipped_already_delivered 5, inserted 0, still 26 rows" || bad "F4: second delivery not idempotent" "$DELIV2"
expect_eq "F4b: user 1 still holds exactly 26 exercises after the replayed delivery" "SELECT count(*)::text FROM public.exercises WHERE user_id='$U1'" "26"
expect_eq "F5: other users received NOTHING (delivery is per-user via auth.uid())" \
  "SELECT count(*)::text FROM public.exercises WHERE user_id<>'$U1' AND catalog_logical_id IN ($FIVE)" "0"
OLD=$(QQ "SET app.uid = '$U2'; SELECT (SELECT (j->>'run_key')||'/'||(j->>'eligible')||'/'||((j->>'inserted')::int + (j->>'skipped_already_delivered')::int + (j->>'skipped_name_collision')::int + CASE WHEN j->>'plank_disposition'='corrected_and_linked_pristine_seed' THEN 1 ELSE 0 END = (j->>'eligible')::int)::text FROM public.deliver_catalog_exercises('$HIST_KEY') j)")
[ "$OLD" = "$HIST_KEY/3/true" ] && ok "F6: the HISTORICAL plank run still delivers for another user (run_key echoed, eligible 3, accounting identity holds) - the new run did not disturb it" || bad "F6: historical run delivery changed" "$OLD"
expect_eq "F7: user 2's plank delivery touched none of the five weight_time identities" "SELECT count(*)::text FROM public.exercises WHERE user_id='$U2' AND catalog_logical_id IN ($FIVE)" "0"
UNAUTH=$(QQ "SELECT public.deliver_catalog_exercises('$NEW_KEY')" | head -1)
[ "$UNAUTH" = "ERROR:  deliver_catalog_exercises: not authenticated" ] && ok "F8: an unauthenticated call is refused (auth.uid() NULL): '$UNAUTH'" || bad "F8: unauthenticated call not refused" "$UNAUTH"
...
[ "$DEL_PRE7" = "ERROR:  deliver_catalog_exercises: no sealed, approved, unrevoked delivery run for this key" ] \
  && ok "F9: DELIVERY BEFORE THE SEAL is refused by the database (staged run, pre-stage-7 clone): '$DEL_PRE7'" || bad "F9: pre-seal delivery not refused" "$DEL_PRE7"
...
[ "$DEL_PRE5" = "ERROR:  deliver_catalog_exercises: no sealed, approved, unrevoked delivery run for this key" ] \
  && ok "F10: DELIVERY BEFORE PUBLICATION is refused too, but ONLY because no run exists yet - publication is NOT a database precondition of delivery; the package sequence (stage 6 demands published content) is what enforces it, stated plainly" || bad "F10: unexpected" "$DEL_PRE5"
...
variant_control "I13 HISTORICAL RUN KEY REUSE" historical_key_stage6 6 pre6 "the chosen run key already exists" "the plank release key used for the new run"
variant_control "I14 MISSING RUN ITEM" missing_member_stage6 6 pre6 "W14E-6 run staging: post-state vector is 8/8/10/3/11/6/2/2/2/10/8" "only four members listed - the post vector gate fires first (10 items, not 11)"
variant_control "I15 EXTRA RUN ITEM" extra_member_stage6 6 pre6 "W14E-6 run staging: post-state vector is 8/8/10/3/11/6/2/2/2/12/8" "the plank identity added as a sixth member - the post vector gate fires first (12 items, not 11)"
variant_control "I15b SWAPPED RUN ITEM (count-camouflaged)" swap_member_stage6 6 pre6 "the membership is not exactly the five governed exercise members" "one governed identity replaced by the plank identity so the count still reads 11 - the MEMBERSHIP gate is what refuses, proving it is live"
variant_control "I16 CARRY INSERTED (stage 6)" carry_stage6 6 pre6 "listed membership identities do not each resolve to exactly one active snapshot" "a deferred carry listed as a member"
variant_control "I17 SABOTAGED FINAL POSTCONDITION (stage 6)" sabotage_post_stage6 6 pre6 "W14E-6 run staging: post-state vector is" "the run and five items were inserted, then rolled back whole"
