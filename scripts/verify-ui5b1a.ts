...
async function main() {
  ...
  console.log('\n6. Target sizing')
  {
    ...
    check('S7: header aligns with the composition (labels at base, sm-only columns)',
      ...
      // RETARGET (W11 — UI_SURFACE): the four/three literal counts are anchored at the closeout tip; W10 renders
      // the headers from the exhaustive COLUMN_HEADERS map — the legacy rows are byte-identical span lists and
      // weight_time adds a fourth RPE column and a third spacer entry (five spacer literals in total).
      (w11AtClosureTip('src/components/workout/WorkoutExerciseBlock.tsx').match(/hidden w-11 sm:inline-block/g) || []).length === 4 &&
      (w11AtClosureTip('src/components/workout/WorkoutExerciseBlock.tsx').match(/w-12 text-center">RPE/g) || []).length === 3 &&
      (block.match(/hidden w-11 sm:inline-block/g) || []).length === 5 &&
      (block.match(/label: 'RPE', className: 'w-12 text-center'/g) || []).length === 4 &&
      block.includes('COLUMN_HEADERS[we.exercise.tracking_mode].map((column, index) =>'))
