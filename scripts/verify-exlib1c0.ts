            ...
            .filter((l) => {
              ...
              if (status === 'M' && f.startsWith('scripts/verify-') && f.endsWith('.ts')) {
                try {
                  ...
                  return !/ADMISSION \(EXLIB-1C0A\)|ADMISSION \(EXLIB-1C0B\)|ADMISSION \(EXLIB-1C0B2\)|ADMISSION \(EXLIB-1C0B3\)|RETARGET \(EXLIB-1C0B3 migration 025 draft\)|ADMISSION \(EXLIB-1C0B4 weight_time product decisions\)|ADMISSION \(EXLIB-1C0B5 weight_time rpe-warmup decision\)/.test(
