...
async function main(): Promise<void> {
  ...
  console.log('\nA. Source refs, inventory, and the candidate (proofs 1-4)')
  {
    ...
    check('A2: exactly ONE numbered migration 027 exists with the exact candidate filename, NO 028, and exactly 27 numbered files forming the contiguous sequence 001-027 (proof 2)',
      (() => {
        ...
        if (files.length !== 29 || !w11Migration028Admitted() || !w14eMigration029Admitted()) return false
        ...
        return JSON.stringify(prefixes) === JSON.stringify(Array.from({ length: 29 }, (_, i) => i + 1))
      })())
  ...
  console.log('\nC. Schema-only application; nothing else changed (proofs 14-15)')
  {
    ...
    check('C2: no Plank content, review, eligibility, seed, inventory, ledger, runtime, API, UI, dependency, or configuration change — the frozen set is blob-identical to the promoted tip and the range through the anchored delivery-runtime predecessor touches only migrations-027/docs/scripts paths (proof 15)',
      (() => {
