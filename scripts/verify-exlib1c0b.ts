  ...
  console.log('\nC. Schema inventory reconciliation (mechanical)')
  {
    ...
    check('C1: exactly the four vocabulary-bearing migrations exist and each is named in the audit; 024 non-interaction stated',
      ...
      JSON.stringify(migFiles) === JSON.stringify([
        ...
        // RETARGET (W14-E — migration 029): the reviewed F-E8 remediation is a NINTH vocabulary-bearing
        // migration purely by carrying the Plank helper's `tracking_mode = 'timed'` / `exercise_type =
        // 'mobility'` invariants verbatim (PREPARED, NOT APPLIED hosted; pinned by w14eMigration029Admitted);
        // the byte-frozen audit still names the four that existed at audit time.
        '029_exlib_plank_cross_run_idempotency.sql']) &&
      w11Migration028Admitted() && w14eMigration029Admitted() &&
      migFiles.filter((f) => !f.startsWith('025') && !f.startsWith('026') && !f.startsWith('027') && !f.startsWith('028') && !f.startsWith('029')).every((f) => audit.includes(f)) &&
