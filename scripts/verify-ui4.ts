...
async function main() {
  ...
  console.log('\n3. Strength PRs')
  {
    ...
    // RETARGET (W11 — UI_SURFACE): the page's inline PR definitions are anchored at the closeout tip; W10.5
    // moved them verbatim into src/lib/recent-pr-tiles.ts (strengthPRTile), which the page delegates to.
    check('S22: established PR definitions preserved verbatim',
      ["e.type === 'weight' ? 'Weight PR'", "'Est. 1RM PR'", "`${e.reps} reps${suffix}`", "e.isUnilateral ? ' per side' : ''"]
        .every((s) => w11AtClosureTip('src/app/(app)/progress/page.tsx').includes(s)) &&
      ["event.type === 'weight' ? 'Weight PR'", "'Est. 1RM PR'", "`${event.reps} reps${suffix}`", "event.isUnilateral ? ' per side' : ''"]
        .every((s) => (require('fs').readFileSync('src/lib/recent-pr-tiles.ts', 'utf8') as string).includes(s)) &&
      page.includes("import { buildRecentPRTiles } from '@/lib/recent-pr-tiles'"))
  }
