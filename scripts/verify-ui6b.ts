...
async function main() {
  ...
  console.log('\n7. Honesty and boundaries')
  {
    ...
    check('G2: educational milestones byte-untouched (lib diff empty)',
      (() => {
        try {
          // ADMISSION (EXLIB-1C0B3): constants.ts gains ONLY the
          // labeled equipment options (pure additions, phase label
          // required); fasting.ts must remain byte-untouched.
          if (execSync('git diff --name-only -- src/lib/fasting.ts',
            { encoding: 'utf8' }).trim() !== '') return false
          const d = execSync('git diff -- src/lib/constants.ts', { encoding: 'utf8' })
          return d === '' ||
