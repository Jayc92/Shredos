-- ============================================================
-- EXLIB-2M MIGRATION-027 APPLY-PREP CANDIDATE
-- 027_exlib_catalog_content_schema.sql
--
-- Reviewed source commit:
--   8289de5ef2f557fced97b9db88647b776a94b1bc
--   (main, tag exlib2l-catalog-content-schema-proposal-reviewed-
--   unapplied, tag object 6301083c9d95caf46e3fe6bb61db9537ae04f1d1)
...
--   retained unchanged alongside this candidate)
--
-- This file is PREPARED FOR A LATER EXPLICIT HOSTED APPLICATION and
-- is NOT APPLIED during EXLIB-2M. Hosted application remains
-- Joseph/ChatGPT-only, on the ShredOS project alone, and only under
-- its own future explicit instruction and evidence trail.
--
-- Migration 027 creates SCHEMA AND LIFECYCLE AUTHORITIES ONLY:
--   - it loads NO identities, snapshots, anatomy, aliases, content,
--     relationships, runs, or membership;
--   - it performs NO human review, admission, publication, approval,
--     seal, revocation, delivery, seed edit, or inventory flip.
-- EXLIB-2K (Plank catalog load) remains DEFERRED until migration 027
-- is reviewed, applied, and evidenced.
--
-- EXECUTABLE-BODY EQUIVALENCE: everything below this header block,
-- beginning at the first executable line (BEGIN;), is mechanically
-- BYTE-IDENTICAL to the reviewed proposal's executable body -
-- including every inline and trailing comment, which retain the
-- reviewed proposal's own wording verbatim (some trailing commentary
-- therefore says "this proposal"; that wording is preserved by the
-- byte-equivalence mandate, and this header is the file's truthful
-- status). No executable statement, constraint, function body,
-- trigger, role, grant, revoke, manifest field, vocabulary,
-- projection rule, lifecycle transition, or rollback boundary was
-- changed, including for formatting.
--
-- Transaction policy (carried from the reviewed proposal's header,
-- which itself follows migrations 023/024/025): ONE explicit
-- top-level transaction encloses every executable statement below.
-- Do not rely on any client to batch.
-- ============================================================
BEGIN;
