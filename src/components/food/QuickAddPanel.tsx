...
function SavedMealRow({ meal, date }: { meal: SavedMeal; date: string }) {
  ...
  return (
    <div>
      <div className={`flex items-center justify-between py-2 border-b border-border/50 last:border-0 ${meal.is_autopilot ? 'py-2.5' : ''}`}>
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {meal.is_autopilot && (
            <Zap className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" aria-label="Autopilot" />
          )}
          <div className="min-w-0">
            <p className={`text-sm font-medium text-foreground truncate ${meal.is_autopilot ? '' : 'text-foreground'}`}>
              {meal.name}
            </p>
  ...
  if (savedMeals.length === 0) {
    return (
      <div className="shred-card text-center py-4 space-y-1">
        ...
        <a href="/food/saved" className="text-xs text-primary hover:underline">
          Create saved meals →
        </a>
      </div>
    )
      ...
      {open && (
        <div>
          ...
          {rest.length > 0 && (
            <div>
              {autopilot.length > 0 && (
                <p className="text-xs text-muted-foreground font-medium mt-3 mb-1">Other saved meals</p>
              )}
