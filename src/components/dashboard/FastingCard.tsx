    ...
    <div className="space-y-3">
      ...
      {milestone && (
        <div className="bg-primary/10 border border-primary/20 rounded-lg px-3 py-2">
          ...
          <p className="text-xs text-muted-foreground mt-0.5">{milestone.note}</p>
        </div>
      )}
    ...
    <div className="shred-card space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-medium text-muted-foreground">Fasting</span>
        </div>
        {!fastingEnabled && (
          ...
          {lastCompletedFast ? (
            <div>
              ...
              {lastCompletedFast.completed_goal !== null && (
                <p className={`text-xs mt-0.5 ${lastCompletedFast.completed_goal ? 'text-green-400' : 'text-muted-foreground'}`}>
                  {lastCompletedFast.completed_goal ? '✓ Goal met' : 'Goal not reached'}
                </p>
              )}
            </div>
          ) : (
            <p className="text-sm text-muted-foreground">No fasts recorded yet.</p>
          )}
