...
/**
 * ShredOS Select — Radix UI based.
 * SelectContent renders via Portal into document.body so it is NEVER
 * clipped or re-ordered by parent stacking contexts.
 * z-[200] ensures it sits above all form content, cards, and overlays.
 *
 * This file intentionally replaces whatever shadcn/Nova installed
 * because the Nova preset's Select can fall back to native <select>
 * behaviour in Safari, causing layout overlap.
 */
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      // Full-width, consistent height, Tailwind design tokens
...
// ── Content — rendered in Portal so it is NEVER clipped ──────────
const SelectContent = React.forwardRef<
  React.ElementRef<typeof SelectPrimitive.Content>,
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Content>
>(({ className, children, position = "popper", ...props }, ref) => (
  // Portal renders into document.body — bypasses any parent overflow/transform
