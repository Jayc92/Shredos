...
export function Sidebar({ fastingEnabled, displayName, className }: SidebarProps) {
  ...
  return (
    ...
    >
      {/* Brand header — the FORGEFIT wordmark lockup (UI-1B polish:
          taller header gives the wordmark room to breathe). */}
        ...
        {NAV_GROUPS.map((group) => {
          ...
          return (
            <div key={group.id} className="pb-2">
              ...
              <div
                aria-hidden="true"
                className="px-3 pb-1.5 pt-3 text-[10px] font-semibold uppercase tracking-[0.14em] text-ink-muted"
              >
                {group.label}
              </div>
                      ...
                      <Link
                        ...
                        className={cn(
                          ...
                          active
                            ...
                            : 'font-medium text-ink-secondary hover:bg-surface-interactive hover:text-ink active:bg-surface-sunken'
                        )}
                      >
                        {active && (
                          <span
                        ...
                        {/* Active icon carries the mint accent — the bar,
                            tint, weight, and aria-current still carry the
                            state; the icon color is additive, never the
                            only cue. */}
                        <Icon
                          className={cn(
                            'shrink-0',
                            route.secondary ? 'h-4 w-4' : 'h-[18px] w-[18px]',
                            active && 'text-brand'
                          )}
                          aria-hidden="true"
                        />
      ...
      <div className="flex-shrink-0 border-t border-edge-subtle p-2">
        ...
        <form action="/api/auth/signout" method="POST">
          <button
            type="submit"
            className="flex w-full items-center gap-2.5 rounded-[var(--radius-control)] px-3 py-2 text-sm font-medium text-ink-secondary transition-colors hover:bg-surface-interactive hover:text-ink active:bg-surface-sunken"
          >
